...
def calculate_occlusion_attributions(model, inputs, sliding_window_shape, baseline):
    # Initialize the Occlusion method
    occlusion = Occlusion(model)

    # Calculate attributions (importance scores) for the inputs using feature occlusion
    attributions = occlusion.attribute(
        inputs,
        sliding_window_shapes=sliding_window_shape,  # Occlude one feature at a time
        baselines=baseline,  # Baseline (zero input)
    )
...
# Call the Occlusion method
sliding_window_shape = (1, 1)  # Occlude one feature at each time step
attributions = calculate_occlusion_attributions(
    model, input_data, sliding_window_shape, baseline_data
)
...
plt.title("Magnitude of Feature Occlusion for Time-Series Prediction")
...
avg_importance = np.mean(importance_matrix, axis=0)[:-1]
print("Average importance score for each feature:")
print(avg_importance)
# Plot the average importance scores
plt.figure(figsize=(10, 4))
plt.barh(feature_labels[:-1], avg_importance)
...
plt.figure(figsize=(10, 4))
plt.plot(avg_cumulative_importance, marker="o")
plt.xlabel("Lag")
plt.ylabel("Lag Importance Score")
# plt.title("Average Cumulative Importance Scores for Processed History")
plt.grid(True)
plt.xticks(np.arange(sequence_length), np.arange(sequence_length, 0, -1))
